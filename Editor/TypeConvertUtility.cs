...
{
    public static T Convert<T, TNew>(T source, params string[] skipFields)
        ...
        where TNew : T
    {
        ...
        if (source is TNew)
        {
            if (UnityEditor.EditorUtility.DisplayDialog("Already Good Enough",
                    string.Format("This already is a 'New {0}'.{1}Do you want to downgrade it to '{0}'?",
                        typeName, Environment.NewLine),
                    "Yes", "No"))
            {
                return Convert<T, TNew>(source, true, skipFields);
            }

            return null;
        }

        return Convert<T, TNew>(source, false, skipFields);
    }
    ...
    private static IEnumerable<KeyValuePair<SerializedObject, string>> FindReferencesTo(Component obj,
        GameObject go)
    {
